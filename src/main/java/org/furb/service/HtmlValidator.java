...
public class HtmlValidator {
    ...
    public boolean isValidTags(String[] input) {
        for (String content : input) {
            ...
            if (!tag.isValidTag())
                throw new InvalidTagException("A tag " + content + " é inválida! Sua estrutura não segue os requisitos");
    ...
    private void updateTagStack(Tag tag) {
        ...
        if (tag.isFinal()) {
            if (tagStack.isEmpty())
                throw new InvalidHtmlStructureException("A tag " + tag.getContent() + " é inválida! Não há tags para fechar");

            Tag lastTag = tagStack.pop();
            String expectedTag = lastTag.getContent().replace("<", "</");
            if (!expectedTag.equalsIgnoreCase(tag.getContent()))
                throw new IllegalTagsSequence("A tag " + tag.getContent() + " é inválida, existe uma tag para fechar ainda! A tag esperada é " + lastTag.getContent().replace("<", "</"));
        }
    }
    ...
    private Tag[] castListToArray() {
        Tag[] tagsFromListObjectList = new Tag[tagList.getSize()];
        for (int i = 0; i < tagList.getSize(); i++) {
            tagsFromListObjectList[i] = tagList.get(i);
        }
        return tagsFromListObjectList;
    }
